...
app.controller("ToolsCtrl", function($scope,$sce) {
	...
	$scope.getWIFKeyFromPrivateKey = function() {
		if ( $scope.privateKeyToWif.length != 64 ) {
			$scope.notifier.danger($translate.instant('NOTIFIER_PRIVATEKEY_LENGTH_CHECK_FAILED') );
		} else {
			var ret = Wallet.getWIFFromPrivateKey($scope.privateKeyToWif);
			if ( ret == -1 || ret == -2 ) {
				$scope.notifier.danger($translate.instant('NOTIFIER_PRIVATEKEY_DECRYPT_FAILED') );
			} else {
				$scope.wif = ret;
			}
		}
