...
app.config([
  '$compileProvider', function($compileProvider) {
    $compileProvider.aHrefSanitizationWhitelist(
        /^\s*(https?|blob|ftp|mailto|tel|file|sms):/)
  }
])
    ...
    link: function(scope, element, attrs) {
      ...
      element.on('change', function(onChangeEvent) {
        ...
        reader.onload = function(onLoadEvent) {
          ...
          var wallet = new Wallet(passwordHash, iv, masterKey, publicKeyHash,
              privateKeyEncrypted)
...
app.controller('ModalInstanceCtrl', function($scope, $modalInstance, items) {
  ...
  if (items.tx.outputs.length == 2) {
    if (Wallet.ToAddress(items.tx.outputs[1].scripthash) !=
        $scope.FromAddress) {
  ...
  $scope.signatureData = function() {
    if ($scope.privateKey.length != 64) {
      $scope.notifier.danger(
          $translate.instant('NOTIFIER_PRIVATEKEY_LENGTH_CHECK_FAILED'))
    } else {
      $scope.signedData = Wallet.signatureData($scope.txRawData,
          $scope.privateKey)
    }
  }
})
...
app.controller('ToolsCtrl', function($scope, $sce) {
  ...
  $scope.getPrivateKey = function() {
    if ($scope.wif.length != 52) {
      $scope.notifier.danger(
          $translate.instant('NOTIFIER_WIF_LENGTH_CHECK_FAILED'))
    } else {
      var ret = Wallet.getPrivateKeyFromWIF($scope.wif)
      if (ret == -1 || ret == -2) {
        $scope.notifier.danger(
            $translate.instant('NOTIFIER_WIF_DECRYPT_FAILED'))
      } else {
  ...
  $scope.getPublicKey = function($encode) {
    if ($scope.privateKey.length != 64) {
      ...
      $scope.publicKey = Wallet.getPublicKey($scope.privateKey, $encode)
                               .toString('hex')
    }
  }
  ...
  $scope.getPublicKeyEncoded = function() {
    if ($scope.publicKey.length != 130) {
      $scope.notifier.danger(
          $translate.instant('NOTIFIER_PUBLICKEY_LENGTH_CHECK_FAILED'))
    } else {
      $scope.publicKeyEncode = Wallet.getPublicKeyEncoded($scope.publicKey)
                                     .toString('hex')
    }
  }
  ...
  $scope.getScript = function() {
    if ($scope.publicKeyEncode.length != 66) {
      $scope.notifier.danger(
          $translate.instant('NOTIFIER_PUBLICKEY_ENCODED_LENGTH_CHECK_FAILED'))
    } else {
  ...
  $scope.getScriptHash = function() {
    if ($scope.script.length != 70) {
      $scope.notifier.danger(
          $translate.instant('NOTIFIER_SCRIPT_LENGTH_CHECK_FAILED'))
    } else {
  ...
  $scope.getAddress = function() {
    if ($scope.scriptHash.length != 40) {
      $scope.notifier.danger(
          $translate.instant('NOTIFIER_SCRIPTHASH_LENGTH_CHECK_FAILED'))
    } else {
...
app.controller('GenerateWalletCtrl', function($scope, $translate, $sce) {
  ...
  $scope.changeCreatePassword2 = function() {
    if ($scope.createPassword2.length >= 8 &&
        $scope.createPassword1 == $scope.createPassword2) {
  ...
  $scope.generateWalletFileFromRandomPrivateKey = function() {
    ...
    var walletBlob = Wallet.generateWalletFileBlob($scope.privateKey,
        $scope.createPassword1)
    $scope.objectURL = window.URL.createObjectURL(
        new Blob([walletBlob], { type: 'application/octet-stream' }))
    $scope.objectName = $scope.objectURL.substring(
        $scope.objectURL.lastIndexOf('/') + 1)
    //$scope.objectName = $scope.objectName.replace( /-/g, "" );

    $scope.notifier.success(
        $translate.instant('NOTIFIER_SUCCESS_GENERATE_THE_WALLET') +
        ' <b>wallet--' + $scope.objectName + '.db3</b>')
  }
...
app.controller('NeoWalletCtrl',
    function($scope, $translate, $http, $sce, $interval, $modal) {
      $scope.wallet = null
      ...
      $scope.showContent = function($wallet) {
        ...
        $scope.notifier.info($translate.instant('NOTIFIER_FILE_SELECTED') +
      ...
      $scope.decryptWallet = function() {
        ...
        try {
          if ($scope.walletType == 'externalsignature') {
            var ret = Wallet.GetAccountsFromPublicKeyEncoded(
                $scope.PublicKeyEncodedData)
            if (ret == -1) {
              $scope.notifier.danger(
                  $translate.instant('NOTIFIER_PUBLICKEY_VERIFY_FAILED'))
              return
            }

            $scope.notifier.info(
                $translate.instant('NOTIFIER_SUCCESS_DECRYPT_THE_WALLET'))
            $scope.accounts = ret
            ...
            if (ret == -1) {
              $scope.notifier.danger($translate.instant(
                  'NOTIFIER_PRIVATEKEY_LENGTH_CHECK_FAILED'))
            } else if (ret) {
              $scope.notifier.info(
                  $translate.instant('NOTIFIER_SUCCESS_DECRYPT_THE_WALLET'))
              $scope.accounts = ret
            ...
            if (ret == -1) {
              ...
              $scope.notifier.danger(
                  $translate.instant('NOTIFIER_WIF_VERIFY_FAILED'))
            } else if (ret) {
              $scope.notifier.info(
                  $translate.instant('NOTIFIER_SUCCESS_DECRYPT_THE_WALLET'))
              $scope.accounts = ret
            ...
            if (ret == -1) {
              $scope.notifier.danger(
                  $translate.instant('NOTIFIER_PASSWORD_VERIFY_FAILED'))
            } else if (ret == -2) {
              $scope.notifier.danger(
                  $translate.instant('NOTIFIER_ACCOUNTS_VERIFY_FAILED'))
            } else {
              $scope.notifier.info(
                  $translate.instant('NOTIFIER_SUCCESS_DECRYPT_THE_WALLET'))
              $scope.accounts = ret
      ...
      $scope.getUnspent = function($address) {
        ...
        $http({
          method: 'GET',
          url: host.webapi_host + ':' + host.webapi_port +
        ...
        $http({
          ...
          if (res.status == 200) {
            if (res.data.result > 0) {
              ...
              $scope.notifier.info(
                  $translate.instant('NOTIFIER_SUCCESS_CONNECTED_TO_NODE') +
                  ' <b>' + $scope.hostInfo[$scope.hostSelectIndex].hostName +
                  '</b>, ' + $translate.instant('NOTIFIER_PROVIDED_BY') +
                  ' <b>' +
                  $scope.hostInfo[$scope.hostSelectIndex].hostProvider +
                  '</b>, ' + $translate.instant('NOTIFIER_NODE_HEIGHT') +
                  ' <b>' + res.data.result + '</b>.')
            } else {
              $scope.notifier.danger(
                  $translate.instant('NOTIFIER_CONNECTED_TO_NODE') + ' <b>' +
                  $scope.hostInfo[$scope.hostSelectIndex].hostName + '</b> ' +
                  $translate.instant('NOTIFIER_FAILURE'))
            }
          } else {
            $scope.notifier.danger(
                $translate.instant('NOTIFIER_CONNECTED_TO_NODE') + ' <b>' +
      ...
      $scope.sendTransactionData = function($txData) {
        ...
        $http({
          method: 'POST',
          url: host.restapi_host + ':' + host.restapi_port,
          data: '{"jsonrpc": "2.0", "method": "sendrawtransaction", "params": ["' +
          $txData + '"], "id": 4}',
          headers: { 'Content-Type': 'application/json' }
        }).then(function(res) {
          if (res.status == 200) {
            ...
            var txhash = reverseArray(hexstring2ab(Wallet.GetTxHash(
                $txData.substring(0, $txData.length - 103 * 2))))
            //var txhash = Wallet.GetTxHash($txData.substring(0,$txData.length-103*2));

            if (res.data.result == true) {
              $scope.notifier.success(
                  $translate.instant('NOTIFIER_TRANSACTION_SUCCESS_TXHASH') +
                  ab2hexstring(txhash) + ' , <a target=\'_blank\' href=\'' +
                  $scope.txBrowseURL + ab2hexstring(txhash) + '\'><b>' +
                  $translate.instant('NOTIFIER_CLICK_HERE') + '</b></a>')
            } else {
              $scope.notifier.danger(
                  $translate.instant('NOTIFIER_SEND_TRANSACTION_FAILED') +
                  res.data.result)
            }
          }
        }).catch(function (err) { console.log(err) })
      ...
      $scope.MakeTxAndSend = function($txUnsignedData) {
        if ($txUnsignedData.length > 0 &&
            $scope.txSignatureData.length == 128) {
          var publicKeyEncoded = $scope.accounts[$scope.accountSelectIndex].publickeyEncoded
          var txRawData = Wallet.AddContract($txUnsignedData,
              $scope.txSignatureData, publicKeyEncoded)

          $scope.sendTransactionData(txRawData)
        } else {
          $scope.notifier.warning(
              $translate.instant('NOTIFIER_INPUT_DATA_CHECK_FAILED'))
        }
      }
      ...
      $scope.issueTransaction = function() {
        if ($scope.issueAsset.issueAssetID.length != 64) {
          $scope.notifier.warning(
              $translate.instant('NOTIFIER_ISSUE_ASSETID_CHECK_FAILED'))
          return
        }

        if ($scope.issueAsset.issueAmount > 100000000) {
          $scope.notifier.warning(
              $translate.instant('NOTIFIER_ISSUE_AMOUNT_CHECK_FAILED'))
          return
        }

        var publicKeyEncoded = $scope.accounts[$scope.accountSelectIndex].publickeyEncoded
        var txData = Wallet.IssueTransaction($scope.issueAsset.issueAssetID,
            $scope.issueAsset.issueAmount, publicKeyEncoded)
      ...
      $scope.registerTransactionUnsigned = function() {
        if ($scope.registerAsset.assetAmount > 100000000) {
          $scope.notifier.warning(
              $translate.instant('NOTIFIER_REGISTER_AMOUNT_CHECK_FAILED'))
          return
        }
        ...
        var publicKeyEncoded = $scope.accounts[$scope.accountSelectIndex].publickeyEncoded
        var txData = Wallet.RegisterTransaction($scope.registerAsset.assetName,
            $scope.registerAsset.assetAmount, publicKeyEncoded)
      ...
      $scope.transferTransactionUnsigned = function() {
        ...
        if (r == null) {
          $scope.notifier.warning(
              $translate.instant('NOTIFIER_AMOUNT_FORMAT_CHECK_FAILED'))
          return false
        }

        if ($scope.Transaction.Amount <= 0) {
          $scope.notifier.warning(
              $translate.instant('NOTIFIER_AMOUNT_MUST_GREATER_ZERO'))
          return false
        }
        ...
        if (parseFloat($scope.coins[$scope.coinSelectIndex].balance) <
            parseFloat($scope.Transaction.Amount)) {
          $scope.notifier.danger(
              $translate.instant('NOTIFIER_NOT_ENOUGH_VALUE') + ', ' +
              $translate.instant('ASSET') + ': ' +
              $scope.coins[$scope.coinSelectIndex].name + ', ' +
              $translate.instant('BALANCE') + ': <b>' +
              $scope.coins[$scope.coinSelectIndex].balance + '</b>, ' +
              $translate.instant('NOTIFIER_SEND_AMOUNT') + ': <b>' +
              $scope.Transaction.Amount + '</b>')
          return false
        }

        var publicKeyEncoded = $scope.accounts[$scope.accountSelectIndex].publickeyEncoded
        var txData = Wallet.TransferTransaction(
            $scope.coins[$scope.coinSelectIndex], publicKeyEncoded,
            $scope.Transaction.ToAddress, $scope.Transaction.Amount)
        if (txData == -1) {
          $scope.notifier.danger(
              $translate.instant('NOTIFIER_ADDRESS_VERIFY_FAILED'))
          return false
        }
